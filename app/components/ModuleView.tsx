...
import type { ModuleConfig, ModuleKey } from "../lib/data";
...
const moduleGuidance: Record<string, { title: string; body: string }> = {
  Projects: { title: "Check sites that are falling behind", body: "Open a project to see progress, money used and the next owner decision." },
  Employees: { title: "Keep the right people on each site", body: "Search by name or site, then open a person to check their assignment." },
  Contractors: { title: "See who is working where", body: "Review active contractors, contract value and the site assigned to them." },
  Tasks: { title: "Handle delayed work first", body: "Use priority and project filters to find work that may affect completion." },
  "Daily Site Reports": { title: "Read today’s site update", body: "Check work completed, workforce, materials, photos and blockers in one place." },
  Attendance: { title: "Confirm who is on site today", body: "Review check-in status and quickly find missing or late team members." },
  "Site Photos": { title: "Verify progress without visiting the site", body: "Open a photo set to see timestamped visual proof from each location." },
  Inventory: { title: "Prevent tomorrow’s material shortage", body: "Start with low-stock items, then confirm stock available at each store." },
  "Material Movement": { title: "Track every material movement", body: "See what came in, what went out and which site received it." },
  Expenses: { title: "Review approvals waiting for you", body: "Open a claim to see the amount, budget position and supporting proof before deciding." },
  Reports: { title: "Choose a report and download it", body: "Use the project and date filters to prepare a clean owner summary." },
  "Users & Roles": { title: "Give each person only the access they need", body: "Review roles and project access before inviting a new user." },
};
...
export function ModuleView({ module, notify, navigate, selectedProject = "All projects" }: { module: ModuleConfig; notify: (message: string) => void; navigate: (key: ModuleKey) => void; selectedProject?: string }) {
  ...
  const guidance = moduleGuidance[module.title] ?? { title: `Review ${module.title.toLowerCase()}`, body: "Use search and filters to find a record, then open it for full details." };

  useEffect(() => {
    const media = window.matchMedia("(max-width: 760px)");
    const syncBestView = () => setView(media.matches ? "cards" : "table");
    syncBestView();
    media.addEventListener("change", syncBestView);
    return () => media.removeEventListener("change", syncBestView);
  }, [module.title]);
  ...
  const hasActiveFilters = query.trim() !== "" || filterValues.some((value, index) => value !== module.filters[index]);

  function clearFilters() {
    setQuery("");
    setFilterValues(module.filters);
  }

  return <>
    <div className="module-head"><div><button className="module-back" onClick={() => navigate("dashboard")}>← Back to overview</button><span className="module-kicker">{module.title}</span><h1>{module.title}</h1><p>{module.subtitle}{selectedProject !== "All projects" ? ` Showing ${selectedProject}.` : ""}</p></div><button className="btn primary module-action" onClick={() => setModal(true)}>＋ <span>{module.action}</span></button></div>
    ...
    <section className="module-guide" aria-labelledby="module-guide-title"><span className="guide-icon"><AppIcon name={module.icon}/></span><div><small>START HERE</small><h2 id="module-guide-title">{guidance.title}</h2><p>{guidance.body}</p></div><button className="btn secondary" onClick={() => setModal(true)}>＋ {module.action}</button></section>
    ...
    <section className="data-card">
      <div className="data-card-title"><div><h2>All {module.title.toLowerCase()}</h2><p>{filteredRows.length} record{filteredRows.length === 1 ? "" : "s"} shown</p></div>{hasActiveFilters && <button onClick={clearFilters}>Clear search & filters</button>}</div>
      <div className="data-tools"><div className="table-search">⌕<input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={`Search ${module.title.toLowerCase()}...`} aria-label={`Search ${module.title}`}/></div><div className="filter-row" aria-label="Filter records"><span>Filter:</span>{module.filters.map((filter, index) => <select key={filter} value={filterValues[index]} onChange={(event) => updateFilter(index, event.target.value)} aria-label={filter}>{(filterChoices[filter] ?? [filter]).map((choice) => <option key={choice}>{choice}</option>)}</select>)}</div><div className="view-toggle" aria-label="Choose view"><button className={view === "table" ? "active" : ""} onClick={() => setView("table")} aria-label="Table view" aria-pressed={view === "table"}>☷</button><button className={view === "cards" ? "active" : ""} onClick={() => setView("cards")} aria-label="Card view" aria-pressed={view === "cards"}>▦</button></div></div>
